...
C     Analytic part of the integrand
      subroutine CHIDeHiggs(sigma,a3,b1,k1,k3,k,kp)
        implicit none
      ...
      double precision dot,dotsum,phi,as,alphas,dothalf
      double precision pi,s,kmax,ncolor,nb,gq,Gf,mt,mp
      double precision mh
      double precision dgd2008,dgdforward,prefac
...
C     Vertex and Numerator
      Numow1=8.*dotsum(k,k1,k,k3)
      Numopw1=8.*dotsum(kp,k1,kp,k3)
      Numow2=8.*dotsum(k,k1,k,k1)*dotsum(k,k3,k,k3)
      Numopw2=8.*dotsum(kp,k1,kp,k1)*dotsum(kp,k3,kp,k3)
...
C     Propagator: denominator!
      Deno=dot(k,k)*dotsum(k,k1,k,k1)*dotsum(k,k3,k,k3)
      Denop=dot(kp,kp)*dotsum(kp,k1,kp,k1)*dotsum(kp,k3,kp,k3)
...
C   -------------------------  Impact Factor  ---------------------------------   C
      ...
      elseif(formfac.EQ.2)then
         ...
         if(event.EQ.1)then
      phip=dgd2008(0.41d0*b1,dot(k,k),dotsum(k,k1,k,k1),dot(k1,k1))
     &*dgd2008(0.41d0*b1,dot(kp,kp),dotsum(kp,k1,kp,k1),dot(k1,k1))
      phipb=dgd2008(0.41d0*abs(a3),dot(k,k),dotsum(k,k3,k,k3)
     &,dot(k3,k3))*dgd2008(0.41d0*abs(a3),dot(kp,kp)
     &,dotsum(kp,k3,kp,k3),dot(k3,k3))

        elseif(event.EQ.2)then
C     Exact definition of dgdforward -> Very slow
C       phip=dgdforward(0.41d0*b1,dothalf(k,k1,k,k1),iglu)
C     &*prefac(k,mskk1,dot(k1,k1),b1)
C     &*dgdforward(0.41d0*b1,dothalf(kp,k1,kp,k1),iglu)
C     &*prefac(skpk1,kp,dot(k1,k1),b1)
C       phipb=dgdforward(0.41d0*abs(a3),dothalf(k,k3,k,k3),iglu)
C     &*prefac(mk,skk3,dot(k3,k3),abs(a3)) 
C     &*dgdforward(0.41d0*abs(a3),dothalf(kp,k3,kp,k3),iglu)
C     &*prefac(mskpk3,kp,dot(k3,k3),abs(a3))
C    Interpolation of dgdforward
       phip=dgdpol(b1,dothalf(k,k1,k,k1))
     &      *prefac(k,mskk1,dot(k1,k1),b1)
     &      *dgdpol(b1,dothalf(kp,k1,kp,k1))
     &      *prefac(skpk1,mkp,dot(k1,k1),b1)
      phipb=dgdpol(abs(a3),dothalf(k,k3,k,k3))
     &      *prefac(mk,skk3,dot(k3,k3),abs(a3)) 
     &      *dgdpol(abs(a3),dothalf(kp,k3,kp,k3))
     &      *prefac(mskpk3,kp,dot(k3,k3),abs(a3))
C      print*,'prefact',prefac(k,mskk1,dot(k1,k1),b1)
        endif
...
C     Durham Sudakov factor
      if(sudaf.EQ.1)then
       if(interpol.EQ.2)then
           Sphi=Dsuda((0.62d0*mh)**2/x,dot(k,k)/xp)
           Sphib=Dsuda((0.62d0*mh)**2/x,dot(kp,kp)/xp)
       elseif(interpol.EQ.1)then
           Sphi=Dsudapol((0.62d0*mh)**2/x,dot(k,k)/xp)
           Sphib=Dsudapol((0.62d0*mh)**2/x,dot(kp,kp)/xp)
       endif  

C     DLA Sudakov form factor
      elseif(sudaf.EQ.2)then
         if(scale.EQ.1)then
       Sphi=Dsuda(mh**2/x,dot(k,k)/xp)
       Sphib=Dsuda(mh**2/x,dot(kp,kp)/xp)
         elseif(scale.EQ.2)then
           Sphi=sqrt(Dsuda(mh**2/x,dot(skk1,skk1)/xp))
     &      *sqrt(Dsuda(mh**2/x,dot(skk3,skk3)/xp))
           Sphib=sqrt(Dsuda(mh**2/x,dot(skpk1,skpk1)/xp))
     &      *sqrt(Dsuda(mh**2/x,dot(skpk3,skpk3)/xp))
         endif
                 
C     Naive Sudakov Factor      
      elseif(sudaf.EQ.3)then

       Sphi=Dexp(-3./pi/2.*as(mh**2/x)*((log(dot(skk3,skk3)/mh**2/xp)
     &*log(dot(skk1,skk1)/mh**2/xp))))
       Sphib=Dexp(-3./pi/2.*as(mh**2/x)*((log(dot(skpk3,skpk3)/mh**2/xp)
     &*log(dot(skpk1,skpk1)/mh**2/xp))))
...
C     Jets Sudakov Factor
     ...
     &      *sqrt(Dsuda(mh**2/x,dot(skpk3,skpk3)/xp))
       ...
       elseif(interpol.EQ.1)then
         if(scale.EQ.1)then
           Sphi=Dsudapol(mh**2/x,dot(k,k)/xp)
           Sphib=Dsudapol(mh**2/x,dot(kp,kp)/xp)
         elseif(scale.EQ.2)then
           Sphi=sqrt(Dsudapol(mh**2/x,dot(skk1,skk1)/xp))
     &      *sqrt(Dsudapol(mh**2/x,dot(skk3,skk3)/xp))
           Sphib=sqrt(Dsudapol(mh**2/x,dot(skpk1,skpk1)/xp))
     &      *sqrt(Dsudapol(mh**2/x,dot(skpk3,skpk3)/xp))
         endif
       endif  
...
C     Vertex and Numerator
      Numow1=8.*dot(k,k)
      Numopw1=8.*dot(kp,kp)
      Numow2=8.*dot(k,k)*dot(k,k)
      Numopw2=8.*dot(kp,kp)*dot(kp,kp)
...
C     Propagator: denominator!
      Deno=dot(k,k)*dot(k,k)*dot(k,k)
      Denop=dot(kp,kp)*dot(kp,kp)*dot(kp,kp)

C   ----------------------- Impact and Sudakov  --------------------------------   C
C    Impact factor
      phip=dgdpol(b1,dot(k,k))*prefac(k,k,dot(k1,k1),b1)
     &*dgdpol(b1,dot(kp,kp))*prefac(kp,kp,dot(k1,k1),b1)
      phipb=dgdpol(abs(a3),dot(k,k))*prefac(mk,mk,dot(k3,k3),abs(a3)) 
     &*dgdpol(abs(a3),dot(kp,kp))*prefac(mkp,mkp,dot(k3,k3),abs(a3))

C    Sudakov form factor
       if(moode.EQ.1)then
           Sphi=sqrt(Dsudapol(mh**2/x,dot(k,k)/xp))
     &      *sqrt(Dsudapol(mh**2/x,dot(k,k)/xp))
           Sphib=sqrt(Dsudapol(mh**2/x,dot(kp,kp)/xp))
     &      *sqrt(Dsudapol(mh**2/x,dot(kp,kp)/xp))
       elseif(moode.EQ.2)then   
C           Rg=1.2d0
           Rg=1.d0
           Sphi=Rg**2*Dsudapol((0.62d0*mh)**2,dot(k,k))
           Sphib=Rg**2*Dsudapol((0.62d0*mh)**2,dot(kp,kp))
       endif
      endif   
...
C     Regge factor
      a1t=0.09d0-0.3d0*dot(k1,k1)/(1.-b1)
      a3t=0.09d0-0.3d0*dot(k3,k3)/(1.+a3)
...
C      print*,'In suda: mu=',(0.62d0*mh)**2/x,' l=',dot(k,k)/xp
C      print*,'sigma=',sigma
C      print*,'******************************'
