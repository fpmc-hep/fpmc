...
namespace resonances0odd_aattbar {
...
//  f0top = mass/(4*M_PI); //maximum allowed phi-ttbar coupling
  f0 = m/sqrt(4*M_PI);
//  f0 = mass*2./(4*M_PI); //maximum allowed phi-gammagamma coupling
  double mtop = 172.76;
//  double width_photon = mass;
  double width_photon = m*m*m / ( 4*M_PI*f0*f0 ); //a->diphoton width
  double w_const = 3/(8*M_PI)*mtop*mtop/(f0top*f0top)*mass*sqrt(1-4*mtop*mtop/(mass*mass)); //assumption for ttbar paper
  ...
  return 3.0*mtop*mtop/((f0*f0top)*(f0*f0top))*s*s*s/((s-mass*mass)*(s-mass*mass)+mass*mass*w_const*w_const);
