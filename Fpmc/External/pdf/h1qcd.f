*-- 17/11/06: KEPKA - implementation of new tabels, see below      
*###########################################################
* Main routine for parameterisations of different qcd fits
* QCD fits to F2D(3)
*
*   Input:   ZT = x_{i/IP}
...
*           IFIT = 20, new ZEUS QCD fit 
*           IFIT = 30, combned H1 and ZEUS QCD fit
*       --- old versions ---       
*           IFIT = 2 (NLO fit as in H1 1994 hep-ex/9708016)
*           IFIT = 5 (LO fits as in H1 1994 hep-ex/9708016)
*           IFIT = 8 (extended version of H1 1994 hep-ph/0602228)
*       --- obsolete ---       
*           IFIT = 1, 3, 4, 6
*           IFIT = 102, 105, 108, 110, 120, 130
*
...
*         Note that these parton distributions are normalised
*         to represent xpom times the 'true' parton distributions 
*         multiplied by the flux factor at xpom integrated 
*         over t in the interval -1.0<t<t_min. 
*         
*###########################################################

      SUBROUTINE QCDFIT(ZT,Q2T,XPQ,IFIT)

c      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER INDICE,IFIT
      data indice/0/
      real*8 zt,q2t
      ! general array for  uds g anti-sdu      
      DOUBLE PRECISION XPQ
      DIMENSION XPQ(-6:6)

      ! for QCD_2006, structure functions - not used
      DOUBLE PRECISION F2(2),FL(2),C2(2),CL(2) !



C Indice is an internal variable: indice=1 old H1 fits
C                                 indice=2 new fits from Laurent
c This is necessary since the tables do not have the same sizes...


      IF(IFIT.GT.0) THEN !ifit turns to zero after first cal1
        indice = 0
        IF(IFIT.EQ.2)      THEN 
                           indice = 1
      ...
      if(indice.eq.1) call QCD_1994(ZT,Q2T,XPQ,IFIT)
      if(indice.eq.2) call QCD_LS(ZT,Q2T,XPQ,IFIT)
      if(indice.eq.3) call QCD_2006(ZT,Q2T,IFIT,XPQ,F2,FL,C2,CL)
      if(indice.eq.4) call QCD_2006(ZT,Q2T,IFIT,XPQ,F2,FL,C2,CL)
      ...
      SUBROUTINE QCD_LS(ZT,Q2T,XPQ,IFIT)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
      INTEGER NQ2,NZ,IFIT
      real*8 zt,q2t
      real*8 uds11,uds12,uds21,uds22
     ...
     &          G_GRID(NZMAX,NQ2MAX)
      ...
      ! no initialization done if IFIT=0
      IF(IFIT.GT.0) CALL I_NLOTABLES(IFIT)
C      IF(IFIT.EQ.8 ) CALL I_NLOH194EXT_LS
c      IF(IFIT.EQ.10) CALL I_NLOH1_LS 
c      IF(IFIT.EQ.20) CALL I_NLOZEUS_LS 
c      IF(IFIT.EQ.30) CALL I_NLOH1ZEUS_LS 
...
C Initialise xpq
      ...
      Q2L = 3.3D0   ! Q2min
      Q2U = 35000.D0 ! Q2max   for LHC
c      Q2U = 150000.D0 ! Q2max
      ZL  = 1.0d-3 ! beta_min
      ZU  = 1.D0   ! beta_max

c  Protect if variables outside limits
      IF (Q2T.GT.Q2U) Q2T = Q2U*0.999999
      IF (Q2T.LT.Q2L) Q2T = Q2L*1.000001
      IF (ZT.GT.ZU) ZT = ZU*0.999999
      IF (ZT.LT.ZL) ZT = ZL*1.000001      


c find indices for beta, q2
      iz=0
      do 1511 i=1,nzmax-1
         if(zt.ge.beta_grid(i).and.zt.le.beta_grid(i+1)) 
     +	      iz=i
1511  continue      
c      print *,'iz : ',iz,beta_grid(iz)
      jq2=0
      do 1512 j=1,nq2max-1
         if(q2t.ge.q2_grid(j).and.q2t.le.q2_grid(j+1)) 
     +	      jq2=j
1512  continue      

c      print *,'jq2 : ',jq2,q2_grid(jq2)

C Light Flavour Singlet
      UDS11 = UDS_GRID(iz,jq2)
      UDS12 = UDS_GRID(iz,jq2+1)
      UDS21 = UDS_GRID(iz+1,jq2)
      UDS22 = UDS_GRID(iz+1,jq2+1)
  
c      print *,'uds:',uds11,uds12,uds21,uds22

C Charm
      C11 = C_GRID(iz,jq2)
      C12 = C_GRID(iz,jq2+1)
      C21 = C_GRID(iz+1,jq2)
      C22 = C_GRID(iz+1,jq2+1)

C Gluon
      G11 = G_GRID(iz,jq2)
      G12 = G_GRID(iz,jq2+1)
      G21 = G_GRID(iz+1,jq2)
      G22 = G_GRID(iz+1,jq2+1)
 
c      print *,'g:',g11,g12,g21,g22

      dlz1 = beta_grid(iz)
      dlz2 = beta_grid(iz+1)
      dlq21 = dlog(q2_grid(jq2)*1.D0)
      dlq22 = dlog(q2_grid(jq2+1)*1.D0)
      dlz = zt
      dlq2 = dlog(q2t*1.D0)
 
c      print *,'kin:',dlz1,dlz2,dlq21,dlq22
c      print *,'kin:',dlz,dlq2
...
C Fill XPQ array
c put charm as light quark for now...
      XPQ(-4) = C_T
c      XPQ(-4) = UDS_T/100.D0
      XPQ(-3) = UDS_T
      XPQ(-2) = UDS_T
      XPQ(-1) = UDS_T
      XPQ( 0) = G_T
...
C Check output
      DO I=-6,6
        IF(XPQ(I).LT.-0.1D-10) THEN
          WRITE(6,*)'Error in H1 LS fit parameterisation',XPQ(I)
          STOP
        ENDIF
      ENDDO
...
c------------------------------------------------------------------ 
     ...
     &          G_GRDL(NZMAX,NQ2MAX)
      COMMON /CPARAMB/ BETA_GRID, Q2_GRID,UDS_GRID,C_GRID,G_GRID 
      INTEGER LUNDAT             ! input channel for reading CMSJET param
      Character *80 CMSJDAT      ! Name of the file
      DATA    LUNDAT /85/, CMSJDAT/'./Tables/pdftable_h194ext.txt'/


      OPEN(LUNDAT,FILE=CMSJDAT,STATUS='OLD')
...
c read input tables and put numbers in grid
      ...
      read(LUNDAT,*) beta_grid(iz_test),q2_grid(iq2_test),
     + uds_grid(iz_test,iq2_test), 
     + g_grid(iz_test,iq2_test),
     + c_grid(iz_test,iq2_test)  
...
c     + c_grid(iz_test,iq2_test)
 ...
 1004 format(2(F14.7,1X),3(F10.5,1X))
...
c------------------------------------------------------------------ 
     ...
     &          G_GRDL(NZMAX,NQ2MAX)
      ...
      INTEGER LUNDAT             ! input channel for reading CMSJET param
      Character *80 CMSJDAT      ! Name of the file
      DATA    LUNDAT /85/, CMSJDAT/'./Tables/pdftable_h197.txt'/
c      DATA    LUNDAT /85/, CMSJDAT/'table_h194.txt'/
...
c___________________________________________________________________ 
     ...
     &          G_GRDL(NZMAX,NQ2MAX)
      ...
      INTEGER LUNDAT             ! input channel for reading CMSJET param
      Character *80 CMSJDAT      ! Name of the file
      DATA    LUNDAT /85/, CMSJDAT/'./Tables/pdftable_h1zeus.txt'/
c      DATA    LUNDAT /85/, CMSJDAT/'table_h194.txt'/
...
c___________________________________________________________________ 
     ...
     &          G_GRDL(NZMAX,NQ2MAX)
      ...
      INTEGER LUNDAT             ! input channel for reading CMSJET param
      Character *80 CMSJDAT      ! Name of the file
      DATA    LUNDAT /85/, CMSJDAT/'./Tables/pdftable_zeus.txt'/
...
c------------------------------------------------------------------- 
      ...
      SUBROUTINE QCD_1994(ZT,Q2T,XPQ,IFIT)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
...
C Define grid
      ...
      Q2U = 100.D0 ! Q2max
      ZL  = 0.01D0 ! beta_min
      ZM  = 0.50D0 ! beta_mid (end of log binning)
      ZU  = 1.D0   ! beta_max
      NQ2 = 30     ! Points in Q2 grid
      NZa = 25     ! Points in log beta grid
      NZ  = 50     ! Total points in grid
      DQ2 = (DLOG(Q2U)-DLOG(Q2L))/DFLOAT(NQ2-1)
      DZa = DLOG(ZM/ZL)/DFLOAT(NZa-1)
      DZb = (ZU-ZM)/DFLOAT(NZ-NZa)

C Check input parameters
C      write(*,*) Q2L,Q2T,ZT,ZU
c CHR 
      IF (Q2T.GT.75.D0) Q2T = 75.D0
cc      IF (Q2T.GT.100.D0) Q2T = 100.D0
      IF (Q2T.LT.Q2L) Q2T = Q2L
C      Q2T=MIN(75.0d0,MAX(Q2L,Q2T))

      IF (ZT.GT.ZU) ZT = ZU
c CHR 
      IF (ZT.LT.0.04D0) ZT = 0.04D0      
cc      IF (ZT.LT.0.01D0) ZT = 0.01D0      
C      ZT =MIN(ZU,MAX(0.04D0,ZT))
...
C Central and boundary z,Q2 points for interpolation
      ...
      IF(IZ.LT.NZa) THEN
        DLZ  = DLOG(ZT) ! Logarithmic interpolation
...
C Fill XPQ array
C OK: 13/01/07 to be compatible with version of DPEMC 2.7,
C we set the c-quark to 0 by default. It used to be set to 0 at
C fpmc.f
...
c      print '(A, 3F8.2)', 'h1qcd', XPQ(-1), XPQ( 0), XPQ( 1)
...
C========================================================================
     ...
     &  XYINTER(X1,X2,Y1,Y2,XT,YT,F11,F12,F21,F22)
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
